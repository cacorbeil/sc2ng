            ...
            "render": function ( data, type, full, meta ) {
               if (type == "display") {
                  if (data.PlusMinus > 0) {
                     ...
                     " <span style='color:"+pColor+"' title='50% winrate'><small>("+pText+data.PlusMinus+")</small></span>";
                  } else {
...
function drawFromStorage(players, table) {
   ...
   players.data.forEach(function(p) {
      ...
      if (p.Updated) {
         ...
         $('#nbPlayers').text('Showing ' + Players.countUpdated() + ' ranked players. ' + (Players.count() - Players.countUpdated()) + ' are unranked.');
      }
   });
};
